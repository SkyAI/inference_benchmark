...
_STDDEV_RGB = [58.395, 57.12, 57.375]
...
def _mean_image_subtraction(image, means, num_channels):
    ...
    stddev =  tf.expand_dims(tf.expand_dims(_STDDEV_RGB, 0), 0)
    return (image - means)/stddev
...
def get_preprocess_fn(model, mode='classification'):
    def process(record):
        ...
        image = preprocess_image(imgdata, INPUT_SIZE, INPUT_SIZE, do_crop=True)
        return image, label
...
def run(frozen_graph, model, data_files, batch_size,
    ...
    def eval_input_fn():
        ...
        dataset = dataset.apply(tf.data.experimental.map_and_batch(map_func=preprocess_fn, batch_size=batch_size, num_parallel_calls=16))
...
if __name__ == '__main__':
    ...
    with g_1.as_default():
        ...
        dataset = dataset.apply(tf.data.experimental.map_and_batch(map_func=get_preprocess_fn(args.model), batch_size=1, num_parallel_calls=8))
    ...
    with tf.Session(graph=g_1) as sess:
        for i in range(TEST_COUNT):
            ...
            #minus 1 to align the 1000 class label
            if results[0][0]-1 in top5classes[i]:
